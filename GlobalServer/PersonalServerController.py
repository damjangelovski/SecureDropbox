...
def addPersonalServerIPadress(request):
    ...
    repo.setIP(request.get(MessageProperty.USERNAME.value), request.get('personal_IP'))

    return jsonify({MessageProperty.MESSAGE_TYPE.value: MessageType.PERSONAL_INIT_OK, MessageProperty.STATUS.value: 'OK'})

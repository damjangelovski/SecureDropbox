...
def deviceInitFromGlobal(request):
    return jsonify({
        MessageProperty.MESSAGE_TYPE.value: MessageType.DEVICE_INIT_OK.value,
...
def deviceInitFromDevice(request):
    if MessageProperty.DEVICE_ID.value not in request:
        ...
        return jsonify({
            MessageProperty.MESSAGE_TYPE.value: MessageType.DEVICE_INIT_OK.value,
            MessageProperty.STATUS.value: 'no device-id'})
    ...
    if MessageProperty.ONE_TIME_PAD.value not in request:
        ...
        return jsonify({
            MessageProperty.MESSAGE_TYPE.value: MessageType.DEVICE_INIT_OK.value,
            MessageProperty.STATUS.value: 'no otp'})
    ...
    addDeviceToLocalStorage(str(request.get(MessageProperty.DEVICE_ID.value)),
...
def connectDevice(request):
    return jsonify({
        MessageProperty.MESSAGE_TYPE.value: MessageType.DEVICE_ONLINE_OK.value,
        MessageProperty.STATUS.value: 'OK'})
    ...
    for row in content:
        ...
        if deviceId == storedDeviceId:
            if devicePublicKey == storedDevicePublicKey:
                return jsonify({
                    MessageProperty.MESSAGE_TYPE.value: MessageType.PERSONAL_INIT_OK.value,
                    MessageProperty.STATUS.value: 'OK'})
            else:
                print('invalid device public key')
                return jsonify({
                    MessageProperty.MESSAGE_TYPE.value: MessageType.PERSONAL_INIT_OK.value,
                    MessageProperty.STATUS.value: 'invalid device public key'})

    print('device not found')
    return jsonify({
        MessageProperty.MESSAGE_TYPE.value: MessageType.PERSONAL_INIT_OK.value,
        MessageProperty.STATUS.value: 'no device found'})
